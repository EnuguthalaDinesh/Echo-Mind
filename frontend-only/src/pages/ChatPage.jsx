...
import { ArrowLeft, Send, Users, Wifi, WifiOff, Plus, MessageCircle, Trash2, Mic, Paperclip, Sparkles, Smile } from 'lucide-react';
...
const ChatPage = () => {
  ...
  // Use user.id (which is now returned by /login) or fall back to email/anon ID.
  ...
  const sendToBackend = async (text) => {
    ...
    const payload = {
      ...
      domain: domain
      // shop_id_for_order_lookup field REMOVED as it caused Pydantic errors
    };
    
    // DEBUG LOG: Display payload to help debug any remaining 422 errors
    ...
    try {
      ...
      if (!currentChatId && data.case_id) setCurrentChatId(data.case_id);
    ...
    <div className={`flex h-screen ${chatBg}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          {chatHistory.map(chat => (
            <motion.div
              ...
              className={`group flex items-center justify-between p-3 rounded-xl glass cursor-pointer transition-colors border border-indigo-200/30 ${currentChatId === chat.id ? 'bg-white/10 border-white' : 'hover:bg-white/20'}`}
      ...
      <div className="flex-1 flex flex-col h-screen relative">
        ...
        >
          ...
          <motion.button
            type="submit"
            whileHover={{ scale: 1.13, backgroundColor: "#c7d2fe" }}
            className="p-3 bg-gradient-to-tr from-indigo-500 to-blue-400 hover:from-indigo-600 text-white rounded-full shadow-lg transition"
          >
            <Send className="h-5 w-5" />
          </motion.button>
          <motion.button
            type="button"
            whileHover={{ scale: 1.11, backgroundColor: "#bbf7d0" }}
            onClick={handleEscalation}
            className="p-3 bg-gradient-to-tr from-teal-400 to-green-400 hover:from-teal-500 text-white rounded-full shadow-md transition"
          >
