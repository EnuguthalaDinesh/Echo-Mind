...
from typing import List, Optional, Tuple, Dict, Union # Added Union for flexible types
...
from pydantic import BaseModel, EmailStr, Field # Added Field for defaults
...
class ChatRequest(BaseModel):
    ...
    domain: str  # expected: general | technical | finance | travel
...
async def on_startup():
    ...
    try:
        if MONGO_URI:
            ...
            app.state.users_collection = app.state.chatbot_db["users"]       # New: For Auth
            app.state.tickets_collection = app.state.chatbot_db["tickets"]   # New: For Tickets
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        pass # Connection is closed automatically on disconnect or policy violation
...
async def oauth_callback(provider: str, request: Request): # Removed db dependency
    if provider not in ("google", "github"):
        raise HTTPException(400, "Unsupported provider")
    client = oauth.create_client(provider)
    ...
    if not existing:
        ...
        new_user = {
            ...
            "role": assigned_role, # Saving role for OAuth users
            "created_at": datetime.utcnow(),
        }
...
def assign_role_by_name(name: str) -> str:
    ...
    # 1. Admin Role Check
    # Match 'admin', 'admin123', 'adminuser', etc.
    if 'admin' in clean_name:
         # Use a slightly more specific check to catch admin names
         if re.search(r'admin\d*|admin[a-z]*', clean_name) or clean_name == "admin":
             return "admin"
    
    # 2. Agent Role Check
    # Match 'agent', 'sharathagent001', 'supportagent'
    if 'agent' in clean_name:
        # Since 'sharath agent 001' was used, check for 'agent' followed by optional digits
...
async def register(user: UserRegister): 
    ...
    new_user = {
        ...
        "role": assigned_role, # <--- ROLE IS SAVED TO MONGODB HERE
        "created_at": datetime.utcnow(),
    }
...
async def login(request: LoginRequest):
    ...
    customer_id = user.get("id") or str(user.get("_id", "no-id")) # Ensure customer_id is sent
    ...
    return {
        ...
        "id": customer_id, # <--- FIXED: Return ID for ChatPage payload construction
        "role": user_role,
        "name": user.get("name"),
...
async def me(current_user: Optional[Dict] = Depends(get_current_user)):
    ...
    return {
        ...
        "role": current_user.get("role", "user") # <-- ROLE IS RETURNED HERE
    }
...
# -------------------------
# Ticket Endpoints (Updated for Agent Dashboard)
# -------------------------
...
async def get_all_open_tickets(
    ...
    try:
        ...
        # Includes 'pending' status for comprehensive agent queue management
...
async def post_ticket_message(
    ...
    try:
        # In a real app, you would fetch customer email/ticket details here and send the email
        logging.info(f"MOCK: Attempting to send email notification to customer for ticket {ticket_id}")
    except Exception as e:
        logging.error(f"MOCK: Failed to send email notification: {e}")
    # --- END MOCK ---
        
    return {"message": "Reply added successfully", "ticket_id": ticket_id, "ok": True}
...
async def get_ticket(ticket_id: str):
    ...
    if col is None:
        raise HTTPException(status_code=500, detail="Ticket database not available.")
    try:
        # Check if ID is a valid ObjectId string before attempting conversion
        is_valid_object_id = len(ticket_id) == 24 and ObjectId.is_valid(ticket_id)
        query_id = ObjectId(ticket_id) if is_valid_object_id else ticket_id
            
        # Query using both _id (ObjectId) and id (string UUID) for robust lookup
...
async def update_ticket_status(
    ...
    current_user: Dict = Depends(get_current_agent_or_admin) # <--- ADDED AUTH CHECK
):
    col = getattr(app.state, "tickets_collection", None)
    if col is None:
        raise HTTPException(status_code=500, detail="Ticket database not available.")
    ...
    try:
        ...
        update_result = await col.update_one(
            {"$or": [{"_id": query_id}, {"id": ticket_id}]},
            {"$set": {"status": update.status.lower(), "updated_at": datetime.utcnow()}}
        )
...
async def chat_interaction(
    ...
    if source_type in ["KB", "Gemini"]:
        # Conditionally create dynamic FAQ entry for future KB hits
        # (Removed create_dynamic_faq logic for brevity, assuming implementation is elsewhere or complex)
    ...
    return ChatResponse(
        bot_response=final_response,
        case_status="resolved", # Mark resolved as we delivered the best response we could
        sentiment_detected=sentiment,
        predicted_domain=predicted_domain,
        intent_confidence=confidence,
        intent_source=source,
